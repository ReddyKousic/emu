...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
	...
	let { published } = data;
	let status: (typeof publishStatusEnum.enumValues)[number] = '' as any;
	if (published === true) {
		status = publishStatusEnum.enumValues[0];
	} else if (published === false) {
		status = publishStatusEnum.enumValues[1];
	} else {
		return json({ error: 'Invalid published status' }, { status: 400 });
	}
